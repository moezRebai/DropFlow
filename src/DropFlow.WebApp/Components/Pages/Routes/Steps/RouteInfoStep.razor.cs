...
public partial class RouteInfoStep : IAsyncDisposable
{
    ...
    private async Task LoadDepots()
    {
        try
        {
            Console.WriteLine("🏢 Loading depots...");
            ...
            if (State.DepotId.HasValue)
            {
                ...
                if (_selectedDepot != null)
                {
                    ...
                    Console.WriteLine($"✅ Depot restored in edit mode by ID: {_selectedDepot.DisplayText}");
                }
                else
                {
                    ...
                    Console.WriteLine($"⚠️ Depot ID {State.DepotId.Value} not found - switching to custom address");
                }
            }
            else if (!string.IsNullOrWhiteSpace(State.DepartureAddress))
            {
                // Pas de DepotId mais adresse définie → essayer plusieurs stratégies de matching
                Console.WriteLine($"🔍 Trying to detect depot from address: {State.DepartureAddress}");
                ...
                {
                    Console.WriteLine($"🔍 Trying GPS matching: {State.DepartureLatitude}, {State.DepartureLongitude}");
                    ...
                    {
                        Console.WriteLine($"✅ Depot matched by GPS coordinates");
                    }
                }
                ...
                if (matchingDepot == null)
                {
                    Console.WriteLine($"🔍 Trying partial address matching...");
                    ...
                    if (matchingDepot != null)
                    {
                        Console.WriteLine($"✅ Depot matched by partial address similarity");
                    }
                }
                ...
                if (matchingDepot != null)
                {
                    ...
                    _isCustomAddress = false;
                    Console.WriteLine($"✅ Depot auto-selected from address: {matchingDepot.DisplayText}");
                }
                else
                {
                    ...
                    _isCustomAddress = true;
                    _selectedDepot = _depotOptions.First(o => o.IsCustomAddress);
                    Console.WriteLine($"✅ Custom address mode - Address: {State.DepartureAddress}");
                }
            }
            else
            {
                ...
                if (defaultDepot != null)
                {
                    ...
                    Console.WriteLine($"✅ Default depot auto-selected: {defaultDepot.DisplayText}");
                }
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"❌ Error loading depots: {ex.Message}");
    ...
    private async Task OnVehicleChanged(VehicleDto? vehicle)
    {
        ...
        if (vehicle != null)
        {
            ...
            Console.WriteLine($"🚚 Vehicle selected: {vehicle.DisplayName}");
            ...
            {
                State.MarkAsChanged();
                Console.WriteLine($"⚠️ Vehicle changed from {previousVehicleId} to {vehicle.Id} - optimization invalidated");
            }
        }
        else
        {
            ...
            Console.WriteLine("⚠️ Vehicle cleared");
        }
    ...
    private async Task OnDepotChanged(DepotOption? option)
    {
        ...
        if (option.IsCustomAddress)
        {
            ...
            Console.WriteLine("📍 Switched to custom address mode");
        ...
        else if (option.Depot != null)
        {
            ...
            Console.WriteLine($"✅ Depot selected: {option.Depot.Name} - {option.Depot.FormattedAddress}");
            ...
            {
                State.MarkAsChanged();
                Console.WriteLine($"⚠️ Depot changed - optimization invalidated");
                Console.WriteLine($"   Previous: {previousAddress}");
                Console.WriteLine($"   Current:  {State.DepartureAddress}");
            }
        }
    ...
    private async Task OnDateChanged(DateTime? newDate)
    {
        if (newDate.HasValue)
        {
            ...
            {
                State.MarkAsChanged();
                Console.WriteLine($"⚠️ Date changed from {previousDate:dd/MM/yyyy} to {newDate:dd/MM/yyyy} - optimization invalidated");
            }
    ...
    private async Task CheckAvailableDeliveriesAsync(DateTime? date)
    {
        ...
        try
        {
            ...
            Console.WriteLine($"📊 {_availableDeliveriesCount} livraisons disponibles pour le {date:dd/MM/yyyy}");
        }
        catch (Exception ex)
        {
            Console.WriteLine($"❌ Erreur lors du comptage des livraisons: {ex.Message}");
    ...
    private async Task InitializeGoogleMapsAutocompleteAsync()
    {
        try
        {
            Console.WriteLine("🗺️ Initializing Google Maps autocomplete...");
            ...
            Console.WriteLine("✅ Google Maps autocomplete initialized");
        }
        catch (Exception ex)
        {
    ...
    public Task OnAddressSelected(GooglePlaceResult place)
    {
        try
        {
            Console.WriteLine($"📍 Address selected: {place.FormattedAddress}");
            ...
            {
                State.MarkAsChanged();
                Console.WriteLine($"⚠️ Custom address changed - optimization invalidated");
                Console.WriteLine($"   Previous: {previousAddress} ({previousLatitude}, {previousLongitude})");
                Console.WriteLine($"   Current:  {State.DepartureAddress} ({State.DepartureLatitude}, {State.DepartureLongitude})");
            }
