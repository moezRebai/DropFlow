...
public partial class CreateDelivery : IAsyncDisposable
{
    ...
    [Inject] private ILogger<CreateDelivery> Logger { get; set; } = default!;
    // ----------------------------------------------------------------
    // PARAMETERS
    // ----------------------------------------------------------------
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        {
            Logger.LogInformation($"? Error loading data: {ex.Message}");
    ...
    private async Task LoadTimeSlotsAsync()
    {
        ...
        {
            Logger.LogInformation($"? Error loading timeslots: {ex.Message}");
    ...
    private async Task<IEnumerable<ClientLookupDto>> SearchClients(string searchTerm, CancellationToken token)
    {
        ...
        {
            Logger.LogInformation($"Error searching clients: {ex.Message}");
            return [];
        }
    }
    ...
    private async Task InitializeGoogleMapsAutocompleteAsync()
    {
        try
        {
            Logger.LogInformation("??? Initializing Google Maps...");
    ...
    private async Task HandleSubmit()
    {
        ...
        {
            Logger.LogInformation($"Error saving delivery: {ex.Message}");
