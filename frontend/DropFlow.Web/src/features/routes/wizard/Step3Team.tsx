    ...
    <div className="flex flex-col gap-6">
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center gap-3 rounded-xl border-2 border-dashed border-slate-200 py-10">
          ...
          <p className="text-sm text-slate-400">Aucun chauffeur disponible pour le {new Date(wizard.date).toLocaleDateString('fr-FR')}</p>
          <p className="text-xs text-slate-400 max-w-64 text-center">Les chauffeurs déjà assignés à des tournées confirmées ne sont pas affichés</p>
        </div>
      ) : (
        <>
          {/* Main driver */}
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-slate-700">Chauffeur principal <span className="text-red-500">*</span></h3>
            </div>
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-semibold text-slate-700">Assistants <span className="text-xs font-normal text-slate-400">(optionnel)</span></h3>
            </div>
            ...
            {helpers.length > 0 && (
              <div className="flex flex-col gap-2">
                {helpers.map(h => {
                  ...
                  return (
                    <div key={h.driverId} className="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
                      ...
                      <p className="flex-1 text-sm font-medium text-slate-700">{h.driverName}</p>
                      <button onClick={() => removeHelper(h.driverId)} className="text-slate-400 hover:text-red-500">
                        <X className="h-4 w-4" />
                      </button>
                    </div>
                  )
              ...
              <div className="flex flex-wrap gap-2">
                {availableForHelper.map(driver => (
                  <button
                    ...
                    className="flex items-center gap-1.5 rounded-lg border border-dashed border-slate-300 px-3 py-1.5 text-xs font-medium text-slate-600 hover:border-sky-400 hover:text-sky-600 transition-colors"
                  >
