    ...
    : IDeliveryService
{
    public async Task<ResponseResult<DeliveryDto>> GetDeliveryByIdAsync(int id)
    {
        try
        {
            var delivery = await context.Deliveries
                ...
                .Include(d => d.Store)
                .Include(d => d.Route) // ? AJOUT�
                .Include(d => d.TimeSlot)
                .Include(d => d.Items)
                .Include(d => d.UrgentDriver) // ? AJOUT�
            ...
            var dto = new DeliveryDto
            {
                ...
                Type = delivery.Type, // ? AJOUT�
                TypeDisplay = delivery.Type.Humanize(), // ? AJOUT�
                ...
                RouteId = delivery.RouteId, // ? AJOUT�
                RouteReference = delivery.Route?.Reference, // ? AJOUT�
                EstimatedArrivalTime = delivery.EstimatedArrivalTime, // ? AJOUT�
                ActualArrivalTime = delivery.ActualArrivalTime, // ? AJOUT�
                ...
                // Urgent
                UrgentDriverId = delivery.UrgentDriverId, // ? AJOUT�
                UrgentDriverName = delivery.UrgentDriver?.User.FullName, // ? AJOUT�
    ...
    public async Task<ResponseResult<int>> CreateDeliveryAsync(CreateDeliveryDto dto)
    {
        try
        {
            ...
            // ? AJOUT� : Validation Type-specific
            if (dto.Type == DeliveryType.Urgent)
            {
                ...
                if (!dto.TimeSlotId.HasValue)
                    return ResponseResult<int>.Failure("Un cr�neau est requis pour une livraison urgente");
                ...
                var driver = await context.Drivers.FirstOrDefaultAsync(dr => dr.Id == dto.UrgentDriverId.Value);
            ...
            var delivery = new Delivery
            {
                ...
                UrgentDriverId = dto.UrgentDriverId, // ? AJOUT�
        ...
        catch (Exception ex)
        {
            ...
            return ResponseResult<int>.Failure($"Erreur lors de la cr�ation : {ex.Message}");
        }
    }
    ...
    public async Task<ResponseResult> UpdateDeliveryAsync(int id, UpdateDeliveryDto dto)
    {
        try
        {
            ...
            // ? AJOUT� : Cannot update if in route sheet (except draft)
            if (delivery.RouteId.HasValue)
            {
                var routeSheet = await context.Routes.FirstOrDefaultAsync(r => r.Id == delivery.RouteId.Value);
                if (routeSheet?.Status != RouteStatus.Draft)
                    return ResponseResult.Failure("Impossible de modifier une livraison dans une tourn�e confirm�e");
            }

            // ? AJOUT� : Type-specific validation
            if (dto.Type == DeliveryType.Urgent)
            {
                if (!dto.UrgentDriverId.HasValue)
                ...
                if (!dto.TimeSlotId.HasValue)
                    return ResponseResult.Failure("Un cr�neau est requis pour une livraison urgente");
            }
            ...
            delivery.UrgentDriverId = dto.UrgentDriverId; // ? AJOUT�
        ...
        catch (Exception ex)
        {
            ...
            return ResponseResult.Failure($"Erreur lors de la mise � jour : {ex.Message}");
        }
    }
    ...
    public async Task<ResponseResult> DeleteDeliveryAsync(int id)
    {
        try
        {
            ...
            if (delivery.Status == DeliveryStatus.Delivered)
                return ResponseResult.Failure("Impossible de supprimer une livraison d�j� effectu�e");

            // ? AJOUT�
            if (delivery.RouteId.HasValue)
                return ResponseResult.Failure("Impossible de supprimer une livraison dans une tourn�e");
            ...
            logger.LogInformation("Livraison {Reference} supprim�e par {UserId}", reference, currentUser.Id);
    ...
    public async Task<ResponseResult> UpdateStatusAsync(int id, DeliveryStatus status)
    {
        try
        {
            ...
            // ? AJOUT� : Auto-fill delivery timestamps
        ...
        {
            logger.LogError(ex, "Erreur lors de la mise � jour du statut");
            return ResponseResult.Failure("Erreur lors de la mise � jour du statut");
        }
    }
    ...
    public async Task<ResponseResult> DuplicateDeliveryAsync(int id)
    {
        try
        {
            ...
            logger.LogInformation("Livraison {Original} dupliqu�e vers {New} par {UserId}",
                original.Reference, reference, currentUser.Id);

            return ResponseResult.Success("Livraison dupliqu�e avec succ�s");
        }
        catch (Exception ex)
        {
    ...
    public async Task<ResponseResult> BulkUpdateStatusAsync(List<int> deliveryIds, DeliveryStatus status)
    {
        try
        {
            ...
            return ResponseResult.Success($"{deliveries.Count} livraison(s) mise(s) � jour");
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error bulk updating status");
            return ResponseResult.Failure("Erreur lors de la mise � jour en masse");
        }
    }
    ...
    public async Task<ResponseResult> BulkDeleteAsync(List<int> deliveryIds)
    {
        try
        {
            ...
            var cannotDelete = deliveries.Where(d =>
                d.Status == DeliveryStatus.Delivered || d.RouteId.HasValue).ToList(); // ? MODIFI�

            if (cannotDelete.Any())
                return ResponseResult.Failure(
                    $"{cannotDelete.Count} livraison(s) ne peut/peuvent pas �tre supprim�e(s)");
            ...
            return ResponseResult.Success($"{deliveries.Count - cannotDelete.Count} livraison(s) supprim�e(s)");
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> IsDeliveryAvailableForRouteAsync(int deliveryId, int? excludeRouteId = null)
    {
        try
        {
            ...
            // Si pas de route assign�e, disponible
            if (delivery.RouteId == null)
                return true;

            // Si c'est la route courante (en �dition), disponible
            if (excludeRouteId.HasValue && delivery.RouteId == excludeRouteId.Value)
                return true;

            // V�rifier le statut de la route
            if (delivery.Route == null)
                return true;

            // ? Disponible si route Draft ou Cancelled
            // ? Verrouill�e si route Confirmed, InProgress, ou Completed
    ...
    public async Task<RouteDto?> GetActiveRouteForDeliveryAsync(int deliveryId)
    {
        try
        {
            ...
            // Si route Draft ou Cancelled, pas verrouill�e
        ...
        int? currentRouteId = null)
    {
        try
        {
            ...
            {
                // En mode �dition : inclure les livraisons de cette route
                query = query.Where(d =>
            ...
            {
                // En mode cr�ation : exclure les livraisons dans routes actives
                query = query.Where(d =>
                        d.RouteId == null || // Pas de route
                        d.Route!.Status == RouteStatus.Draft || // Route Draft
                        d.Route!.Status == RouteStatus.Cancelled // Route Cancelled
            ...
            var dtos = deliveries.Select(d => new DeliveryDto
            {
                ...
                // Route info (pour affichage si d�j� assign�e)
                RouteId = d.RouteId,
        ...
        catch (Exception ex)
        {
            ...
            return ResponseResult<List<DeliveryDto>>.Failure(
                "Erreur lors de la r�cup�ration des livraisons disponibles");
        }
    }
    ...
    public async Task<ResponseResult<DeliveryDto>> GeocodeDeliveryAsync(int id)
    {
        ...
        catch (Exception ex)
        {
            ...
            return ResponseResult<DeliveryDto>.Failure($"Erreur lors du g�ocodage : {ex.Message}");
        }
    }
