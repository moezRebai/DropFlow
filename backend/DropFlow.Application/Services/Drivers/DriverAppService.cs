    ...
    : IDriverAppService
{
    ...
    public async Task<ResponseResult<bool>> StartRouteAsync(int routeId)
    {
        try
        {
            ...
            if (route.Date.Date != DateTime.UtcNow.Date)
                return ResponseResult<bool>.Failure("Vous ne pouvez démarrer qu'une tournée prévue pour aujourd'hui");
    ...
    public async Task<ResponseResult<bool>> CompleteRouteAsync(int routeId)
    {
        try
        {
            ...
            var pending = route.Deliveries.Count(d => d.Status == DeliveryStatus.InProgress);
            if (pending > 0)
                return ResponseResult<bool>.Failure(
                    $"{pending} livraison(s) encore en cours. Chaque livraison doit être validée ou annulée avant de terminer la tournée.");
    ...
    // ═══════════════════════════════════════════════════════════
    // DASHBOARD (today + upcoming en un seul appel)
    // ═══════════════════════════════════════════════════════════

    public async Task<DriverDashboardResponse> GetDashboardAsync()
    {
        var todayRoute = await GetTodayRouteAsync();
        var upcomingRoutes = await GetUpcomingRoutesAsync();
    ...
    public async Task<List<DriverRouteSummaryDto>> GetUpcomingRoutesAsync()
    {
        try
        {
            var driver = await GetCurrentDriverAsync();
            ...
            return routes.Select(r => new DriverRouteSummaryDto
            {
                RouteId = r.Id,
                Reference = r.Reference,
                Date = r.Date,
                Status = (int)r.Status,
                StatusDisplay = r.Status.Humanize(),
                VehicleName = $"{r.Vehicle.Brand} {r.Vehicle.Model}",
                TotalDeliveries = r.TotalDeliveries,
                DeliveredCount = r.Deliveries.Count(d => d.Status == DeliveryStatus.Delivered),
        ...
        {
            logger.LogError(ex, "Error getting upcoming routes for driver");
            return [];
        }
    }

    public async Task<DriverTodayResponse> GetRouteDetailAsync(int routeId)
    {
        try
        {
            var driver = await GetCurrentDriverAsync();
            if (driver == null)
                return new DriverTodayResponse { HasRoute = false, Message = "Compte livreur non configuré." };

            var route = await context.Routes
                .Include(r => r.Vehicle)
                .Include(r => r.Team).ThenInclude(t => t.Driver).ThenInclude(d => d.User)
                .Include(r => r.Deliveries).ThenInclude(d => d.Client)
                .Include(r => r.Deliveries).ThenInclude(d => d.ClientAddress)
                .Include(r => r.Deliveries).ThenInclude(d => d.TimeSlot)
                .Include(r => r.Deliveries).ThenInclude(d => d.Items)
                .Where(r => r.Id == routeId)
                .Where(r => r.Team.Any(t => t.DriverId == driver.Id))
                .FirstOrDefaultAsync();

            if (route == null)
                return new DriverTodayResponse { HasRoute = false, Message = "Tournée introuvable." };
            ...
            var dto = new DriverRouteDto
            {
                ...
                TotalDistanceKm = route.TotalDistance,
                TotalDurationMinutes = route.TotalDuration,
                TeamMembers = route.Team.Select(t => t.Driver.User.FullName).ToList(),
                Deliveries = route.Deliveries
                    .Where(d => d.SequenceOrder.HasValue)
                    .OrderBy(d => d.SequenceOrder)
                    .Select(d => new DriverDeliveryListDto
            ...
            return new DriverTodayResponse { HasRoute = true, Route = dto };
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error getting route detail {RouteId}", routeId);
            return new DriverTodayResponse { HasRoute = false, Message = "Erreur lors du chargement." };
        }
    }
    ...
    public async Task<DriverHistoryResponse> GetDeliveryHistoryAsync(int page, int pageSize)
    {
        try
        {
            var driver = await GetCurrentDriverAsync();
            ...
            var query = context.Deliveries
                ...
                .Include(d => d.Route)
                    .ThenInclude(r => r!.Team)
                .Where(d => d.Route != null && d.Route.Team.Any(t => t.DriverId == driver.Id))
                .Where(d => d.Status == DeliveryStatus.Delivered || d.Status == DeliveryStatus.Canceled)
            ...
            return new DriverHistoryResponse
            {
                ...
                Deliveries = deliveries.Select(d => new DriverHistoryDeliveryDto
                {
                    Id = d.Id,
                    Reference = d.Reference,
                    Date = d.ScheduledDate,
                    ClientName = d.Client.DisplayName,
                    City = d.ClientAddress?.City ?? string.Empty,
                    Status = (int)d.Status,
                    StatusDisplay = d.Status.Humanize(),
                    IsClientAbsent = d.IsClientAbsent,
                    DeliveredDateTime = d.DeliveredDateTime,
                    RouteReference = d.Route?.Reference ?? string.Empty
                }).ToList()
            };
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "Error getting delivery history for driver");
            return new DriverHistoryResponse();
        }
    }
