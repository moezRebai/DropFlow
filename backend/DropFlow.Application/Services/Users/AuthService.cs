    ...
    : IAuthService
{
    ...
    public async Task<AuthResult> RegisterTenantAsync(RegisterTenantDto dto)
    {
        try
        {
            ...
            if (existingTenant != null)
                return new AuthResult(false, Message: "Ce nom d'entreprise est d�j� utilis�");
    ...
    public async Task<AuthResult> LoginAsync(LoginDto dto)
    {
        ...
        if (!user.IsActive)
            return new AuthResult(false, Message: "Votre compte est d�sactiv�");
        ...
        if (user.TenantId != TenantIds.DropFlowAdmin)
        {
            ...
            if (userTenant is not { IsActive: true })
                return new AuthResult(false, Message: "Votre entreprise est d�sactiv�e");
        }
    ...
    public async Task<AuthResult> AcceptInvitationAsync(AcceptInvitationDto dto)
    {
        try
        {
            ...
            if (!invitation.IsValid())
                return new AuthResult(false, Message: "Cette invitation a expir�");
    ...
    public async Task<ResponseResult> ForgotPasswordAsync(ForgotPasswordDto dto)
    {
        try
        {
            ...
            // IMPORTANT : Pour des raisons de s�curit�, on retourne toujours succ�s
            if (!users.Any())
            {
                logger.LogWarning(
    ...
    public async Task<AuthResult> RefreshTokenAsync(string refreshToken)
    {
        try
        {
            ...
            if (stored == null || !stored.IsActive)
                return new AuthResult(false, Message: "Refresh token invalide ou expir�");
