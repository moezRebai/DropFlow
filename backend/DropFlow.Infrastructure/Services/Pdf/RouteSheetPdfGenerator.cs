...
public class RouteSheetPdfGenerator(
    IHttpClientFactory httpClientFactory,
    ILogger<RouteSheetPdfGenerator> logger) : IRouteSheetPdfGenerator
{
    // ── Neutral palette ──────────────────────────────────────────
    ...
    public byte[] Generate(RouteSheetDto data) => GenerateCore(data, logoBytes: null);

    public async Task<byte[]> GenerateAsync(RouteSheetDto data)
    {
        byte[]? logoBytes = null;
        if (!string.IsNullOrEmpty(data.CompanyLogoUrl))
            logoBytes = await DownloadImageAsync(data.CompanyLogoUrl);
        return GenerateCore(data, logoBytes);
    }

    private byte[] GenerateCore(RouteSheetDto data, byte[]? logoBytes)
    {
    ...
    private async Task<byte[]?> DownloadImageAsync(string url)
    {
        try
        {
            var client = httpClientFactory.CreateClient("PdfImages");
        ...
        {
            logger.LogWarning(ex, "Failed to download logo from {Url}", url);
            return null;
        }
    }
    ...
    private void ComposeHeader(IContainer container, RouteSheetDto data, byte[]? logoBytes)
    {
