...
{
    /// <summary>
    /// Retourne la tournée du jour + la liste des tournées à venir en un seul appel
    /// </summary>
    [HttpGet("dashboard")]
    [ProducesResponseType(typeof(DriverDashboardResponse), 200)]
    public async Task<IActionResult> GetDashboard()
    {
        var result = await driverAppService.GetDashboardAsync();
        return Ok(result);
    }
    ...
    /// <summary>
    /// Retourne l'historique des livraisons effectuées/annulées du livreur (paginé)
    /// </summary>
    [HttpGet("deliveries/history")]
    [ProducesResponseType(typeof(DriverHistoryResponse), 200)]
    public async Task<IActionResult> GetDeliveryHistory([FromQuery] int page = 1, [FromQuery] int pageSize = 20)
    {
        var result = await driverAppService.GetDeliveryHistoryAsync(page, pageSize);
        return Ok(result);
    }

    /// <summary>
    /// Retourne la liste des tournées à venir (aujourd'hui + futur) du livreur connecté
    /// </summary>
    [HttpGet("routes/upcoming")]
    [ProducesResponseType(typeof(List<DriverRouteSummaryDto>), 200)]
    public async Task<IActionResult> GetUpcomingRoutes()
    {
        var result = await driverAppService.GetUpcomingRoutesAsync();
        return Ok(result);
    }

    /// <summary>
    /// Retourne le détail complet d'une tournée (livraisons incluses) pour le livreur connecté
    /// </summary>
    ...
    {
        var result = await driverAppService.GetRouteDetailAsync(id);
        if (!result.HasRoute)
